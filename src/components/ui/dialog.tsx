...
function DialogOverlay({
  ...
  return (
    <DialogPrimitive.Backdrop
      ...
      /* Adjusted overlay opacity for deeper dark mode feel */
      className={cn(
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Popup
        ...
        /* Swapped ring-foreground/10 for ring-border and ensured bg-popover text color */
        className={cn(
    "fixed top-1/2 left-1/2 z-50 grid w-full max-w-[calc(100%-2rem)] -translate-x-1/2 -translate-y-1/2 gap-4 rounded-xl bg-popover p-6 text-sm text-popover-foreground ring-1 ring-border duration-100 outline-none sm:max-w-sm data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95 shadow-2xl transition-colors",
    className
  )}
        {...props}
      >
...
function DialogTitle({ className, ...props }: DialogPrimitive.Title.Props) {
  return (
    <DialogPrimitive.Title
      ...
      /* Explicitly ensures title is readable in dark mode */
      className={cn(
